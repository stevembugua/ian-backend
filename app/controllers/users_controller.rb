class UsersController < ApplicationController
    ...
    def register
        ...
        if user.valid?
            ...
            app_response(message: 'Registration was succesful',status: :created, data: user)
                ...
                app_response(message: 'Something went wrong during registartion',status: :uprocessable_entity, data: user.errors)
        end
    end

    def logout 
        remove_user
        app_response(message: 'logout succesful')
    end
    ...
    def login
        ...
        if user&.authenticate(user_params[:password])
            save_user(id: user.id)
            app_response(message: 'Login was succesful', status: :ok, data: user)
         else
